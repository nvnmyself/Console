...
export default class Games extends Component {
  ...
  constructor(props) {
    ...
    super(props);
    this.state = {
      date:new Date().getDate()+ '/' + (new Date().getMonth()+1<'10'?'0'+(new Date().getMonth()+1):new Date().getMonth()+8) + '/' + new Date().getFullYear() ,
      userID:'',
      gameID:'',
      isLoading:true,
...
apply=()=>{
  ...
  AddMoney = ()=>{
    ...
    formBody = formBody.join("&");
    //POST request 
    fetch('http://www.radicaltechsupport.com/pubg/activity.php?method=championship', {
      method: "POST",//Request Type 
      body: formBody,//post body 
      headers: {//Header Defination 
        'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
  ...
  ListEmpty = () => {
    return (
      ...
      <View style={[styles.cardHeader,{marginTop:-30}]}>   
        <Text style={{fontWeight:'bold',fontSize:20,color:'black'}}><Ionicons name="rupee"/>--</Text>
        <Text style={{fontWeight:'bold',fontSize:20,color:'black'}}><Ionicons name="rupee"/>--</Text>
  ...
  {this.state.connection_Status==="Offline"?   <View style={{width:'50%',height:40,backgroundColor:'green',alignSelf:'center'}}>
      <Text style={{alignSelf:"center",padding:10}}>You are {this.state.connection_Status}</Text>
        ...
        <FlatList style={styles.list}
          ...
          color="#2196F3"
        /> 
        <Text>{item.spot-item.slots} spots left</Text>
               </View>
                <View style={[styles.cardFooter,{marginTop:-30}]}>
                  <View style={{alignItems:"center", justifyContent:"center"}}>
                    {/* <Text style={styles.name}>{item.name}</Text> */}
